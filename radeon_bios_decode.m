...
static int
rhdAtomAnalyzeRomHdr(
            RadeonBios* radeonBios,
		     ...
		     unsigned int *command_offset)
{
    ...
    //<wrapper>
    radeonBios.subsystemVendorId = [NSNumber numberWithInt:le16_to_cpu(hdr->usSubsystemVendorID)];
    radeonBios.subsystemId = [NSNumber numberWithInt:le16_to_cpu(hdr->usSubsystemID)];
    radeonBios.ioBaseAddress = [NSNumber numberWithInt:le16_to_cpu(hdr->usIoBaseAddress)];
    radeonBios.filename = [NSString stringWithCString:rombase + le16_to_cpu(hdr->usConfigFilenameOffset) encoding:NSASCIIStringEncoding];
    radeonBios.biosBootupMessage = [NSString stringWithCString:rombase + le16_to_cpu(hdr->usBIOS_BootupMessageOffset) encoding:NSASCIIStringEncoding];
    //</wrapper>
...
static Bool
rhdAtomAnalyzeCommandDataTable(unsigned char* base,
                                 ATOM_MASTER_COMMAND_TABLE* table,
                        atomCommandTablesPtr data)
{
    ATOM_MASTER_LIST_OF_COMMAND_TABLES* command_table = &table->ListOfCommandTables;
    unsigned short size;
    
    if(!rhdAtomAnalyzeCommonHdr(&table->sHeader))
        return FALSE;
    if(!rhdAtomGetTableRevisionAndSize(&table->sHeader, NULL, NULL, &size))
        return FALSE;
    
# define SET_COMMAND_TABLE(x) {\
rhdAtomAnalyzeRomDataTable(base,command_table->x,(void *)(&(data->x)),&size); \
}
    
# define SET_COMMAND_TABLE_VERS(x) {\
rhdAtomAnalyzeRomDataTable(base,command_table->x,&(data->x.base),&size); \
}
    
    SET_COMMAND_TABLE_VERS(SelectCRTC_Source);
    
    SET_COMMAND_TABLE_VERS(DIG1EncoderControl);
    SET_COMMAND_TABLE_VERS(DIG2EncoderControl);
    
# undef SET_COMMAND_TABLE
# undef SET_COMMAND_TABLE_VERS
...
struct atom_context {
    ...
    atomCommandTables *atomCommandPtr;
};
...
static Bool
show_connectors_from_obj(RadeonBios* radeonBios,
                         ...
                         NSError** error)
{
    ...
    radeonBios.connectors = connectors;
    //</wrapper
	...
	for (i = 0; i < path_obj->ucNumOfDispPath; i++) {
		...
		if (device_support & le16_to_cpu(path->usDeviceTag)) {
            ...
            //<wrapper>
            RadeonBiosConnector* connector = [[RadeonBiosConnector alloc] init];
            [connectors addObject:connector];
            
            connector.objectId = [NSNumber numberWithInt:con_obj_id];
            connector.index = [NSNumber numberWithInt:i];
            connector.type = [NSString stringWithCString:connectortype_to_name(connector_type) encoding:NSASCIIStringEncoding];
            //</wrapper
			...
			for (j = 0; j < ((le16_to_cpu(path->usSize) - 8) / 2); j++) {
				...
				if (grph_obj_type == GRAPH_OBJECT_TYPE_ENCODER) {
                    //EDIT
                    ATOM_OBJECT *encoder = &encoder_obj->asObjects[j];
                    
                    //NSLog(@"encoder id = %d", le16_to_cpu(encoder->usObjectID));
                    //</EDIT
...
//					printf("\tEncoder enum: %x\n", encoder_obj);
                    ...
                    //<wrapper>
                    connector.encoder = [NSString stringWithCString:encoder_name(grph_obj_id) encoding:NSASCIIStringEncoding];
                    //</wrapper>
                     ...
                     */
                    //dual link ja/nein?
                    if(strcmp(encoder_name(grph_obj_id), "INTERNAL_UNIPHY0") == 0) {
					...
					if (le16_to_cpu(path->usConnObjectId) ==
						...
						while (record->ucRecordType > 0
							...
							case ATOM_I2C_RECORD_TYPE:
                                ...
                                //<wrapper>
                                    connector.i2cid = [NSNumber numberWithInt:i2c_config->ucAccess];
                                    connector.osxSenseId = [NSNumber numberWithInt:(i2c_config->ucAccess & 0xF) + 1];
                                //</wrapper>
...
static Bool
show_connectors(RadeonBios* radeonBios,
                struct atom_context *ctx,
		unsigned char *base, ATOM_ROM_HEADER *atom_rom_hdr,
		unsigned int data_offset,
                NSError** error)
...
static void
analyze_pci(RadeonBios* radeonBios,
            unsigned char *pcidata)
{
    ...
    //<wrapper>
    radeonBios.vendorId = [NSNumber numberWithInt:le16_to_cpu(maj)];
    radeonBios.deviceId = [NSNumber numberWithInt:le16_to_cpu(minor)];
    //</wrapper>
}


static inline void radeon_transmitters_find(struct atom_context *ctx) {
    int headerIndex = GetIndexIntoMasterTable(COMMAND, DIG1EncoderControl);
    uint8_t crev, frev;
    uint16_t data_offset;
    unsigned short size;
    
    if(!atom_parse_data_header(ctx, headerIndex, &size, &frev, &crev, &data_offset))
        return;
    
    ATOM_COMMON_ROM_COMMAND_TABLE_HEADER *header = (ATOM_COMMON_ROM_COMMAND_TABLE_HEADER*)(ctx->bios + data_offset);
    
#if 0
    int objectHeaderIndex = GetIndexIntoMasterTable(DATA, Object_Header); //data table index
    uint16_t data_offset;
    unsigned short size; //größe der table
    uint8_t crev, frev; //was is das?
    
    if(!atom_parse_data_header(ctx, objectHeaderIndex, &size, &frev, &crev, &data_offset))
        return;
    
    ATOM_OBJECT_HEADER *object_header = (ATOM_OBJECT_HEADER*)(ctx->bios + data_offset);
    
    //extract header vars
    
    int deviceSupport = le16_to_cpu(object_header->usDeviceSupport);
    ATOM_DISPLAY_OBJECT_PATH_TABLE *displayObjectPathTable = (ATOM_DISPLAY_OBJECT_PATH_TABLE*)
        (ctx->bios + data_offset + le16_to_cpu(object_header->usDisplayPathTableOffset));
    
    uint8_t path_size = 0; //einstiegspunkt für displayobject
    for(int i = 0; i < displayObjectPathTable->ucNumOfDispPath; i++) {
        //get object
        uint8_t *addr = (uint8_t *) displayObjectPathTable->asDispPath + path_size;
        ATOM_DISPLAY_OBJECT_PATH *displayObjectPath = (ATOM_DISPLAY_OBJECT_PATH*)addr;
        path_size += le16_to_cpu(displayObjectPath->usSize); //change offset for next loop entry
        
        NSLog(@"addr=%p", addr);
        
        //check if device is supported?
        if(!(deviceSupport & le16_to_cpu(displayObjectPath->usDeviceTag)))
            continue;
        
        //create loop pointer
        
        //loop through elements in path
        /**
         graphicObjIdsEndOffset = usSize (Größe von Objekt) -  4*(restliche struktur bis auf usGraphicObjIds)
         graphicObjIdLength = graphicObjIdsEndoffset / sizeof(element)
         */
        uint8_t graphicsObjIdsLength = (le16_to_cpu(displayObjectPath->usSize) - 4 * sizeof(USHORT)) / sizeof(USHORT);
        for(int j = 0; j < graphicsObjIdsLength; j++) {
            uint8_t grph_obj_id = (le16_to_cpu(displayObjectPath->usGraphicObjIds[j]) & OBJECT_ID_MASK) >> OBJECT_ID_SHIFT;
            uint8_t grph_obj_num = (le16_to_cpu(displayObjectPath->usGraphicObjIds[j]) & ENUM_ID_MASK) >> ENUM_ID_SHIFT;
            uint8_t grph_obj_type = (le16_to_cpu(displayObjectPath->usGraphicObjIds[j]) & OBJECT_TYPE_MASK) >> OBJECT_TYPE_SHIFT;

            NSLog(@"grph_obj_id = %2x, grph_obj_num = %2x, grph_obj_type = %2x", grph_obj_id, grph_obj_num, grph_obj_type);
            ...
            NSError** error)
{
    ...
    atomDataTables *atomDataPtr;
    atomCommandTables *atomCommandPtr = calloc(sizeof(atomCommandTables),1);

    atomDataPtr = ctx->atomDataPtr;
    atomCommandPtr = ctx->atomCommandPtr;
    
    if (atom_romhdr_off + sizeof(ATOM_ROM_HEADER) > BIOSImageSize) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBAtomRomHeaderExtendsBeyondBiosImage userInfo:nil];
	fprintf(stderr,
    ...
    if (memcmp("ATOM",&atom_rom_hdr->uaFirmWareSignature,4)) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBNoAtomSignature userInfo:nil];
        fprintf(stderr,"%s: No AtomBios signature found\n",
		   __func__);
        return FALSE;
    }
    printf("ATOM BIOS Rom: \n");
    if (!rhdAtomAnalyzeRomHdr(radeonBios, base, atom_rom_hdr, &data_offset, cmd_offset)) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBRomHeaderInvalid userInfo:nil];
        fprintf( stderr, "RomHeader invalid\n");
        return FALSE;
    }
    analyze_pci(radeonBios, base + le16_to_cpu(atom_rom_hdr->usPCI_InfoOffset));

    if (data_offset + sizeof (ATOM_MASTER_DATA_TABLE) > BIOSImageSize) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBAtomDataTableOutsideBios userInfo:nil];
	fprintf(stderr,"%s: Atom data table outside of BIOS\n",
		   __func__);
    }

    if (*cmd_offset + sizeof (ATOM_MASTER_COMMAND_TABLE) > BIOSImageSize) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBAtomCommandTableOutsideBios userInfo:nil];
    ...
    if (!rhdAtomAnalyzeMasterDataTable(base, (ATOM_MASTER_DATA_TABLE *)
				       (base + data_offset),
				       atomDataPtr)) {
        *error = [NSError errorWithDomain:RadeonBiosErrorDomain code:RBROMMasterTableInvalid userInfo:nil];
        fprintf(stderr, "%s: ROM Master Table invalid\n",
		   __func__);
        return FALSE;
    }
    
    if(!rhdAtomAnalyzeCommandDataTable(base, (ATOM_MASTER_COMMAND_TABLE *)
                                       (base + *cmd_offset),
                                       atomCommandPtr))
    {
        fprintf(stderr, "%s: COMMAND SCHAS");
    }
...
#define ATOM_ROM_DATA_PTR	0x20
    ...
    //show_multimedia(radeonBios, ctx, base, atom_rom_hdr, error);

    //EDIT
    
    //    SELECT_CRTC_SOURCE_PARAMETERS_V2* crtcSource = ctx->atomCommandPtr->SelectCRTC_Source.crtcSourceParameters_v2;
    //
    //    NSLog(@"ucCRTC = %2x, ucEncoderID = %2x, ucEncoderMode = %2x, ucPadding = %2x", crtcSource->ucCRTC, crtcSource->ucEncoderID, crtcSource->ucEncodeMode, crtcSource->ucPadding);
    //
    //    ATOM_ASIC_INTERNAL_SS_INFO* asic = ctx->atomDataPtr->ASIC_InternalSS_Info;
    //
    //    for(int i = 0; i < 4; i++) {
    //        ATOM_ASIC_SS_ASSIGNMENT* assignment = &asic->asSpreadSpectrum[i];
    //
    //
    //    }
    // SELECT_CRTC_SOURCE_PARAMETERS_V2 ucEncoderID --> ASIC_TRANSMITTER_INFO ucEncoderID
    radeon_transmitters_find(ctx);
    ///EDIT
    
    //if (!show_connectors(radeonBios, ctx, base, atom_rom_hdr, data_offset, error)) { //NO IDEA what happens here
	show_connectors_from_obj(radeonBios, ctx, base, atom_rom_hdr, error);
    //}
...
BOOL radeon_bios_decode_wrapper(RadeonBios* radeonBios, NSString* path, NSError** error) {
    unsigned offset;
    atomDataTables atomData;
    atomCommandTables atomCommand;
    struct atom_context ctx;
    
    
    NSData* biosData = [NSData dataWithContentsOfFile:path options:NSDataReadingMappedAlways error:error];
    
    memset(&atomData, 0, sizeof(atomData));
    ctx.bios = (unsigned char*)[biosData bytes];
    ctx.atomDataPtr = &atomData;
    ctx.atomCommandPtr = &atomCommand;
    
    Bool success = rhdAtomGetDataTable(radeonBios, &ctx, ctx.bios, &offset, ROMLEN, error);
    
    
    
//    if(success)
//        success = rhdAtomGetCommandTable(radeonBios, &ctx, offset, error);
//    
    return success;
}
